...
async function main() {
  ...
  if (toRemoveCount > 5) {
    console.log(
      "There are " +
        toRemoveCount +
        " not-cleaned releases, but every time one is added, older ones are cleaned, so normally there should be at most one to remove. This is very suspicious and it may indicate that the selected set of releases is too broad. Cleaning has been abandoned and the situation should be investigated manually."
    );
    return;
  }

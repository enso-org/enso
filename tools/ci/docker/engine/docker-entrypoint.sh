...
LANGUAGE_SERVER_YDOC_HOSTNAME=$INTERFACE /opt/enso/bin/enso $PROFILING_OPTIONS --log-level "$LOG_LEVEL" --rpc-port $RPC_PORT --root-id "$LS_ROOT_ID" --interface "$INTERFACE" "$@"

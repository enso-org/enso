...
public record MethodParameter(int index, String name, String tpe, List<String> annotations) {
  /**
   * Returns a parameter's declaration, consisting of its type and name. If the parameter represents
   * a vararg, the declaration is repeated. Otherwise the method returns a single element Stream of
  ...
  public Stream<String> declaredParameters(Optional<Integer> expand) {
    ...
    String paramAnnotations =
        annotations.isEmpty() ? "" : (StringUtils.joinWith(" ", annotations.toArray()) + " ");
  ...
  /**
   * Returns a parameter's name. If the parameter represents a vararg, the name is repeated.
   * Otherwise return a single element Stream of declarations.
   *
   * @param expand For a non-empty value n, the parameter must be repeated n-times.
  ...
  public Stream<String> names(Optional<Integer> expand) {
    return expand
        .map(e -> IntStream.range(0, e).mapToObj(i -> name + "_" + (i + 1)))
        .orElse(Stream.of(name));
  }

  /**
   * Returns a parameter's name when used in the body of the method. Compared to {@link
   * MethodParameter#names} it will take into account potential translation to Java host
   * representation.
   *
   * @param expand For a non-empty value n, the parameter must be repeated n-times.
  ...
  private static final String TruffleDSLPkgAnnotation = "@com.oracle.truffle.api.dsl";

  /**
   * Check if the parameter contains a Truffle-DSL annotation. Parameters annotated with such
   * annotations should not appear in the abstract `execute` method, but should be passed to the
   * specialized method signatures.
   *
   * @return
   */
  public boolean isTruffleInjectedParam() {
    return !annotations.stream()
        .filter(a -> a.startsWith(TruffleDSLPkgAnnotation))
        .findAny()
        .isEmpty();
  }
  ...
  private static final List<String> typesToInject =
      List.of("org.enso.interpreter.runtime.EnsoContext");
}

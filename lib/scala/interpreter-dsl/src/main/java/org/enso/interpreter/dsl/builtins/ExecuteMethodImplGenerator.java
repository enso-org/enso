...
public final class ExecuteMethodImplGenerator extends MethodGenerator {
    ...
    public ExecuteMethodImplGenerator(ExecutableElement method, boolean convertToGuestValue, int expandedVarargs, SafeWrapException[] exceptionWrappers) {
        this(method,
    ...
    protected Optional<Integer> expandVararg(int paramsLen, int paramIndex) {
        return needsVarargExpansion && paramsLen >= (paramIndex + 1) ? Optional.of(varargExpansion) : Optional.empty();
    }
    ...
    private String[] bodyBase(String name, String owner, List<MethodParameter> params) {
        ...
        if (params.isEmpty()) {
            ...
            paramsApplied =
                    StringUtils.join(params.stream().flatMap(x -> x.paramUseNames(expandVararg(paramsLen, x.index()))).toArray(), ", ");
        }
        if (isConstructor) {
    ...
    public List<String> generate(String name, String owner, Map<String, Integer> builtinTypesParameterCounts) {
        ...
        List<MethodParameter> params =  IntStream.range(0, method.getParameters().size()).mapToObj(i ->
                fromVariableElementToMethodParameter(i, rawParams.get(i))).collect(Collectors.toList());

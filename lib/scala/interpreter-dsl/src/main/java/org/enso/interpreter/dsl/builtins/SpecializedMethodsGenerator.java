...
public final class SpecializedMethodsGenerator extends MethodGenerator {
     ...
     *
     * @param specializedParamIdx optional index of parameter on which specialization occurs.
     *                            Used optionally for ordering of specialized methods to satisfy @Specialize pre-conditions
    ...
    private SpecializationMeta inferExecuteParameters() {
        ...
        paramss.forEach((k, v) -> {
            if (v.size() != elements.size()) {
                throw new RuntimeException("Restriction: Specialized methods have to have equal number of parameters.\n"
                                           + "Expected " + elements.size() + ", got " + v.size());
            }

 ...
 */
public record LoggingServer(
    int port, Map<String, Appender> appenders, String appender, boolean start, LogToFile logToFile)
  ...
  public static LoggingServer parse(Config config) throws MissingConfigurationField {
    ...
    LogToFile logToFile =
        config.hasPath(LoggingServiceConfig.logToFileKey)
            ? LogToFile.fromConfig(config.getConfig(LoggingServiceConfig.logToFileKey))
            : LogToFile.disabled();

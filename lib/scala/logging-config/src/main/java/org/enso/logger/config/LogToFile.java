...
/**
 * If `enabled`, all logs up to `logLevel` level will be persisted in the log file in parallel to
...
public record LogToFile(boolean enabled, Level logLevel) {
  ...
  public static LogToFile fromConfig(Config config) {
    if (config.hasPath(logToFileEnabledKey)) {
      Level lvl =
          config.hasPath(logToFileLogLevelKey)
              ? fromConfigValue(config.getString(logToFileLogLevelKey))
              : Level.DEBUG;

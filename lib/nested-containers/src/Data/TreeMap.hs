...
deleteAtSubbranch (k:ks) t = t & fromJust id (fmap (set (at k) . justIfNotNullBranch) (deleteAtSubbranch ks <$> t ^. at k))
...
instance (IsValue t, s ~ Item (TreeMap t k a)) => Convertible (TreeMap t k a) [s] where
    convert = paths
instance (Ord k, s ~ Item (TreeMap t k a))  => Convertible [s] (SparseTreeMap k a) where
    convert []             = mempty
    convert ((ks, a) : ls) = insert ks a $ convert ls
instance (Ord k, Default a, s ~ Item (SolidTreeMap k a)) => Convertible [s] (SolidTreeMap k a) where
    convert []             = mempty
    convert ((ks, a) : ls) = insertDef def ks a $ convert ls

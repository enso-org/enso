...
public class SheetUtil {
  ...
  /**
   * Excel measures columns in units of 1/256th of a character width but the docs say nothing about
   * what particular character is used. '0' looks to be a good choice.
   */
  private static final char defaultChar = '0';

  /**
   * This is the multiple that the font height is scaled by when determining the boundary of rotated
   * text.
   */
  private static final double fontHeightMultiple = 2.0;

  /**
   * Dummy formula evaluator that does nothing. YK: The only reason of having this class is that
   * {@link
   * org.apache.poi.ss.usermodel.DataFormatter#formatCellValue(org.apache.poi.ss.usermodel.Cell)}
   * returns formula string for formula cells. Dummy evaluator makes it to format the cached formula
   * result.
   *
   * <p>See Bugzilla #50021
   */
  ...
  // private static java.awt.font.FontRenderContext fontRenderContext = new
  // java.awt.font.FontRenderContext(null, true, true);

  /**
   * A system property which can be enabled to not fail when the font-system is not available on the
   * current machine. Since POI 5.4.0, this flag is enabled by default.
   */
   ...
   */
  public static double getCellWidth(
      Cell cell, float defaultCharWidth, DataFormatter formatter, boolean useMergedCells) {
   ...
   * Compute width of a single cell
   *
   * <p>This method receives the list of merged regions as querying it from the cell/sheet is
   * time-consuming and thus caching the list across cells speeds up certain operations
   * considerably.
   *
   * @param cell the cell whose width is to be calculated
   * @param defaultCharWidth the width of a single character
   * @param formatter formatter used to prepare the text to be measured
   * @param useMergedCells whether to use merged cells
   * @param mergedRegions The list of merged regions as received via
  ...
  public static double getCellWidth(
    ...
    // We should only be checking merged regions if useMergedCells is true. Why are we doing this
    // for-loop?
    int colspan = 1;
  ...
  /**
   * Calculate the best-fit width for a cell If a merged cell spans multiple columns, evenly
   * distribute the column width among those columns
   *
   * @param defaultCharWidth the width of a character using the default font in a workbook
   * @param colspan the number of columns that is spanned by the cell (1 if the cell is not part of
   *     a merged region)
   * @param style the cell style, which contains text rotation and indention information needed to
   *     compute the cell width
   * @param minWidth the minimum best-fit width. This algorithm will only return values greater than
   *     or equal to the minimum width.
  ...
  private static double getCellWidth(
    ...
    //                return FAILOVER_FUNCTION.apply(defaultCharWidth, colspan, style, minWidth,
    // str);
    //            }
    //            throw t;
    //        }
    ...
    //            /*
    //             * Transform the text using a scale so that its height is increased by a multiple
    // of the leading,
    //             * and then rotate the text before computing the bounds. The scale results in some
    // whitespace around
    //             * the unrotated top and bottom of the text that normally wouldn't be present if
    // unscaled, but
  ...
  /**
   * Compute width of a column and return the result. Note that this fall can fail if you do not
   * have the right fonts installed in your OS.
   *
  ...
  public static double getColumnWidth(Sheet sheet, int column, boolean useMergedCells) {
    return getColumnWidth(
        sheet, column, useMergedCells, sheet.getFirstRowNum(), sheet.getLastRowNum());
  }

  /**
   * Compute width of a column based on a subset of the rows and return the result. Note that this
   * fall can fail if you do not have the right fonts installed in your OS.
   *
   * @param sheet the sheet to calculate
   * @param column 0-based index of the column
   * @param useMergedCells whether to use merged cells
  ...
  public static double getColumnWidth(
    ...
    for (int rowIdx = firstRow; rowIdx <= lastRow; ++rowIdx) {
      ...
      if (row != null) {
        double cellWidth =
            getColumnWidthForRow(
                row, column, defaultCharWidth, formatter, useMergedCells, mergedRegions);
  ...
  /**
   * Get default character width using the Workbook's default font. Note that this can fail if your
   * OS does not have the right fonts installed.
   *
   * @param wb the workbook to get the default character width from
   * @return default character width in pixels
   * @deprecated since POI 5.2.5, it is recommended to switch to {@link
   *     #getDefaultCharWidthAsFloat(Workbook)}.
   */
  @Internal
  ...
  public static float getDefaultCharWidthAsFloat(final Workbook wb) {
    ...
    //        try {
    //            java.awt.font.TextLayout layout = new java.awt.font.TextLayout(str.getIterator(),
    // fontRenderContext);
  ...
  private static final Function5Arity<Float, Integer, CellStyle, Double, AttributedString, Float>
      DEFAULT_FAILOVER_FUNCTION =
          (defaultCharWidth, colspan, style, minWidth, string) -> defaultCharWidth;

  private static Function5Arity<Float, Integer, CellStyle, Double, AttributedString, Float>
      FAILOVER_FUNCTION = DEFAULT_FAILOVER_FUNCTION;

  /**
   * Define a function to be used as a failover when the font system is not available. This function
   * will be called with the default character width, the colspan, the cell style, the minimum width
   * and the attributed string of the cell.
   *
   * <p>The default function will return the default character width.
   *
   * @since POI 5.4.1
   */
  public static void setFailoverFunction(
      Function5Arity<Float, Integer, CellStyle, Double, AttributedString, Float> failoverFunction) {
  ...
  /**
   * Compute width of a single cell in a row Convenience method for {@link #getCellWidth}
   *
   ...
   *
   * <p>If a Cell uses a Font which is not available on the operating system then Java may fail to
   * return useful Font metrics and thus lead to an auto-computed size of 0.
   *
   * <p>This method allows to check if computing the sizes for a given Font will succeed or not.
   *
  ...
  public static boolean canComputeColumnWidth(Font font) {
    //        // not sure what is the best value sample-here, only "1" did not work on some
    // platforms...
  ...
  //        str.addAttribute(java.awt.font.TextAttribute.FAMILY, font.getFontName(), startIdx,
  // endIdx);
  //        str.addAttribute(java.awt.font.TextAttribute.SIZE, (float)font.getFontHeightInPoints());
  //        if (font.getBold()) str.addAttribute(java.awt.font.TextAttribute.WEIGHT,
  // java.awt.font.TextAttribute.WEIGHT_BOLD, startIdx, endIdx);
  //        if (font.getItalic() ) str.addAttribute(java.awt.font.TextAttribute.POSTURE,
  // java.awt.font.TextAttribute.POSTURE_OBLIQUE, startIdx, endIdx);
  //        if (font.getUnderline() == Font.U_SINGLE )
  // str.addAttribute(java.awt.font.TextAttribute.UNDERLINE,
  // java.awt.font.TextAttribute.UNDERLINE_ON, startIdx, endIdx);
  //    }

  /**
   * Return the cell, without taking account of merged regions.
   *
   * <p>Use {@link #getCellWithMerges(Sheet, int, int)} if you want the top left cell from merged
   * regions instead when the reference is a merged cell.
   *
   * <p>Use this where you want to know if the given cell is explicitly defined or not.
   *
  ...
  /**
   * Return the cell, taking account of merged regions. Allows you to find the cell who's contents
   * are shown in a given position in the sheet.
   *
   * <p>If the cell at the given co-ordinates is a merged cell, this will return the primary
   * (top-left) most cell of the merged region.
   *
   * <p>If the cell at the given co-ordinates is not in a merged region, then will return the cell
   * itself.
   *
   * <p>If there is no cell defined at the given co-ordinates, will return null.
   *
   * @param sheet The workbook sheet to look at.
   * @param rowIx The 0-based index of the row.
   * @param colIx The 0-based index of the cell.
  ...
  //    }
  //
  //    protected static void setFontRenderContext(java.awt.font.FontRenderContext
  // fontRenderContext) {

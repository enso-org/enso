...
//     }
//
//     /// Process the `input` event. Events are strings uniquely identifying the source of the
// event,     /// like "key_a", or "mouse_button_1". The `press` parameter is set to true if it was
// a press     /// event, and is set to false in case of a release event.
...
// /// a nondeterministic finite automata (NFA) is created. On the first usage, is it being
// optimized /// to a deterministic finite automata (DFA), which provides a constant time to get
// results on every /// key stroke. This allows us to define very complex shortcuts (regexp-like)
// and keep high /// performance of their resolution. However, the first stage of this approach is
// costly, namely /// building the NFA and optimizing it to DFA. Please refer to the benchmarks in
// this library to /// compare between available implementations.
// ///
// /// Please note, that this registry type allows for broader expression input syntax than what is
// /// described in the `Registry` documentation. In particular, it allows for expressions like
// `-a`, /// which means, that they will be triggered every time the key `a` is released, no matter
// if other /// keys were pressed or not.

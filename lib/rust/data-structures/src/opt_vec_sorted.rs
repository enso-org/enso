...
// FIXME: finish it and document properly.
...
#[derive(Clone, Debug)]
pub struct OptVecSorted<T, Index = usize> {
    items:    Vec<Option<T>>,
...
impl<T, I: Index> OptVecSorted<T, I> {
...
impl<T, I: Index> std::ops::Index<I> for OptVecSorted<T, I> {
    type Output = T;
...
impl<T, I: Index> std::ops::IndexMut<I> for OptVecSorted<T, I> {
...
impl<'a, T, I: Index> IntoIterator for &'a OptVecSorted<T, I> {
    type Item = &'a T;
...
impl<'a, T, I: Index> IntoIterator for &'a mut OptVecSorted<T, I> {
    type Item = &'a mut T;
...
mod tests {
    ...
    #[test]
    fn test_add() {
        let mut v = OptVecSorted::<usize>::new();
        assert!(v.is_empty());

...
use enso_shapely::impl_clone_ref_as_clone;
use enso_shapely::root_call_path;
use fail::*;
use hash::*;
pub use item::*;
use leak::Leak;
...
fn Immutable<T>(data: T) -> Immutable<T> {
    Immutable { data }
}
...
trait RefInto<T> = where for<'t> &'t Self: Into<T>;

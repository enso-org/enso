...
// ///
// /// For example, the macro `if ... then ... else ...` contains three segments and no prefix. On
// the /// other hand, the macro `... -> ...` contains one segment (starting with the `->` token)
// and a /// prefix (it consumes tokens on the left of its first segment).
// ///
...
//     /// The pattern in this field will be matched from right to left, unlike patterns in
// segments.     pub rev_prefix_pattern: Option<Pattern>,
...
// // =========================
//
// /// Definition of macro segment. Contains header, such as `if`, or `->` and pattern that
// following /// tokens have to match.
// #[derive(Clone, Debug)]
// #[allow(missing_docs)]
...
// // ===================
//
// /// Macro allowing for nice macro [`Definition`] generation. For example, the following code
// defines /// the `if ... then .. else ...` macro:
// ///
// /// ```text
// /// macro_definition! {

...
///       ├─ wasm-pack                 | Wasm-pack artifacts. Wasm-pack re-creates it on every run.
///       │  ├─ pkg.js                 | Wasm-pack JS bundle to load WASM and glue it with snippets.
...
///          ├─ app.cjs                | The main JS bundle to load WASM and JS wasm-pack bundles.
///          ├─ shader-extractor.cjs   | NodeJS program to extract non optimized shaders from WASM.
///          ├─ main.js                | The `pks.js` artifact of wasm-pack with bundled snippets.
///          ├─ main.wasm              | The `pks_bg.wasm` artifact of wasm-pack.
...
impl Paths {
    pub async fn new() -> Result<Self> {
        ...
        p.target.ensogl_pack.dist.app = p.target.ensogl_pack.dist.join("app.cjs");
        p.target.ensogl_pack.dist.shader_extractor =
            p.target.ensogl_pack.dist.join("shader-extractor.cjs");
        p.target.ensogl_pack.dist.main_js = p.target.ensogl_pack.dist.join("main.js");
        p.target.ensogl_pack.dist.main_wasm = p.target.ensogl_pack.dist.join("main.wasm");

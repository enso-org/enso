...
macro_rules! define_events {
    ( $( $js_event:ident :: $name:ident ),* $(,)? ) => {$(
        ...
        pub struct $name {
            ...
            raw   : enso_web::$js_event,
            shape : Shape,
        }
        impl $name {

            /// Constructor.
            pub fn new(raw:enso_web::$js_event,shape:Shape) -> Self {
                Self {raw,shape}
            }
            ...
            fn try_get_current_target_element(&self) -> Option<enso_web::Element> {
                let target  = self.current_target()?;
                target.value_of().dyn_into::<enso_web::Element>().ok()
            }

            /// Return the position relative to the given element.
            ///
            /// Note: causes reflow of the JS layout.
            pub fn relative_position_with_reflow(&self, element:&enso_web::Element) -> Vector2<f32> {
        ...
        impl AsRef<enso_web::Event> for $name {
            fn as_ref(&self) -> &enso_web::Event {
                let js_event = AsRef::<enso_web::$js_event>::as_ref(self);
                js_event.as_ref()
            }
        }
    )*};

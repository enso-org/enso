...
impl SpriteSystem {
    ...
    pub fn new(label: &'static str, alignment: alignment::Dim2) -> Self {
        ...
        let alignment = symbol.borrow_variables_mut().add_or_panic("alignment", initial_alignment);
    ...
    pub fn set_geometry_material<M: Into<Material>>(&self, material: M) {
        self.symbol.borrow_shader_mut().set_geometry_material(material);
    }

    /// Sets the surface material for all sprites in this system.
    pub fn set_material<M: Into<Material>>(&self, material: M) {
        self.symbol.borrow_shader_mut().set_material(material);
    }
}
...
impl SpriteSystem {
    ...
    fn init_shader(&self) {
        let mut shader = self.symbol.borrow_shader_mut();

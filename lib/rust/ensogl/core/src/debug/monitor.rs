...
// // ===========
//
// /// Dom elements of the monitor. Please note that it uses `Rc` to both implement cheap copy as
// well /// as to use `Drop` to clean the HTML when not used anymore.
...
//         let width = self.config.labels_width + self.config.results_width + 3.0 *
// self.config.margin;         dom.context.set_fill_style(&self.config.background_color);
...
// // =============
//
// /// A single element in the `Monitor`. It can display labels, values and plots. Each `Panel`
...
//     }
//
//     /// Fetch the measured value from stats, using the panel's sampler, then post-process the
// value     /// to make it useful for displaying on a human-readable graph.
...
// // ===============
//
// /// Abstraction for a sampling utility. Samplers gather the data and expose it in a way suitable
// for /// the monitor.
...
//     /// when displaying the final value. The final value will be the average of # previous
// values.     fn smooth_range(&self) -> usize {
//         2
//     }
//
...
//                 // Before the first frame, all stat values will be 0, which in (only) this case
// is                 // correct.
...
// stats_sampler!("Sprite system count", SpriteSystemCount, sprite_system_count, 100.0, 500.0, 0,
// 1.0); stats_sampler!("Symbol count", SymbolCount, symbol_count, 100.0, 500.0, 0, 1.0);

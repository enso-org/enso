...
module Foreign.Storable.TH (module Foreign.Storable.TH, module X) where
...
class HasDerivs  a where derivs  :: Lens' a [DerivClause]
...
splitTermName n = if (head == term) then Just tail else Nothing where
    head = take len n
    tail = drop len n
    len  = length term
    term = "Term"
...
data DataInfo = DataInfo { _diName  :: TH.Name
                         , _diTVars :: [TH.TyVarBndr]
                         , _diCons  :: [TH.Con]
                         } deriving (Show)

reifyDataInfo :: Name -> Q DataInfo
reifyDataInfo ty = do
    reified <- reify ty
    let errMsg = "***error*** reifyDataInfo: Can only be apllied to Data Types."
    case reified of
        TyConI tyCon -> case tyCon of
            DataD    _ nm tyVars _ cs _ -> return $ DataInfo nm tyVars cs
            NewtypeD _ nm tyVars _ c  _ -> return $ DataInfo nm tyVars [c]
            _  -> fail errMsg
        _ -> fail errMsg

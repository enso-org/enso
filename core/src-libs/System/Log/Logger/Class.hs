...
import qualified Control.Monad.State.Dependent as DS -- FIXME[WD]: Maybe we could replace the defs with this State?
...
instance MonadLogging m => MonadLogging (EitherT e m)
instance MonadLogging m => MonadLogging (DS.StateT s m)

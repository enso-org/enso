...
import           Data.TreeMap (TreeMap, JustValue, IsValue, IsSparseValue, mkVal, checkVal, branches)
...
instance IsValue t => ToList   (TreeSet t k) where toList = paths
instance Ord k => FromList (SparseTreeSet k) where
    fromList []        = mempty
    fromList (ks : ls) = insert ks $ fromList ls
instance Ord k => FromList (SolidTreeSet k) where
    fromList []        = mempty
    fromList (ks : ls) = insertAllLevels ks $ fromList ls

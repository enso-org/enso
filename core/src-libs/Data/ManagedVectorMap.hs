...
unsafeMerge :: (Ord k, PrimMonad m) => ManagedVectorMap k a -> ManagedVectorMapRefM m k a -> m ([(Idx, Idx)], [k])
unsafeMerge st@(ManagedMap _ _ m) store = do
    ...
    let unknownKeys  = localKeys List.\\ foreignKeys
    ...
    return (ixMapping, unknownKeys)

emptyFromStructure :: (Ord k, PrimMonad m) => ManagedVectorMapRefM m k a -> m (ManagedVectorMapRefM m k a)
emptyFromStructure store = do
    toInit <- keys store
    new    <- empty
    forM_ toInit $ flip unsafeAddKey new
    return new
...
data    UnsafeWorld
newtype UnsafeSTRef a = UnsafeSTRef (STRef UnsafeWorld a)
makeWrapped ''UnsafeSTRef

newUnsafeSTRef :: forall m a. PrimMonad m => a -> m (UnsafeSTRef a)
newUnsafeSTRef = fmap wrap . fmap unsafeCoerce . newSTRef

readUnsafeSTRef :: forall m a. PrimMonad m => UnsafeSTRef a -> m a
readUnsafeSTRef = readSTRef . unsafeCoerce . unwrap

writeUnsafeSTRef :: forall m a. PrimMonad m => UnsafeSTRef a -> a -> m ()
writeUnsafeSTRef ref = writeSTRef $ unsafeCoerce $ unwrap ref

modifyUnsafeSTRef :: forall m a. PrimMonad m => UnsafeSTRef a -> (a -> a) -> m ()
modifyUnsafeSTRef ref = modifySTRef'_ $ unsafeCoerce $ unwrap ref

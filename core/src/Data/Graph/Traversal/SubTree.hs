...
getSubTree :: Fold.Foldable SubTreeDiscovery m a => a -> m [Component.Any]
getSubTree = \a -> Fold.buildFold @SubTreeDiscovery a $! pure mempty ; {-# INLINE getSubTree #-}
...
instance ( MonadIO m
         , Fold.Foldable SubTreeDiscovery m Node.Some
    ...
    buildLayerFold = \tpLink acc -> do
        ...
        if tp == tgt then acc' else Fold.buildFold @SubTreeDiscovery tp acc'

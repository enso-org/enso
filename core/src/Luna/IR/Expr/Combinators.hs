...
narrowAtom :: forall a m. (MonadPass m, KnownType (AtomOf a), Reader LAYER (ExprLayer Model) m)
...
-- deleteSubtree :: forall l m. ( MonadPass m, Editors NET '[EXPR, LINK' EXPR] m
...
--     else return ()
--
-- changeSource :: forall l m. (MonadPass m, Editors NET '[LINK' EXPR] m, Editors LAYER '[ExprLayer Succs, ExprLinkLayer Model] m)

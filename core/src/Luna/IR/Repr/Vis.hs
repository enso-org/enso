...
snapshot :: (IRMonad m, MonadVis m, Editors LAYER '[ExprLayer UID, ExprLayer Type, ExprLayer Model, ExprLinkLayer UID, ExprLinkLayer Model] m, Editor NET EXPR m)
         => Prelude.String -> m ()
snapshot title = do
    ts  <- exprs
...
visNode2 :: (IRMonad m, Editors LAYER '[ExprLayer UID, ExprLayer Type, ExprLayer Model, ExprLinkLayer UID, ExprLinkLayer Model] m)

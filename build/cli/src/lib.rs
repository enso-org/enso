...
impl Processor {
    ...
    pub fn build_new_ide(
        ...
        let build_old_gui = self.get(gui);
        let build_new_gui = self.get(params.gui);
        let gui = async move {
            let (old_gui, new_gui) = futures::future::join(build_old_gui, build_new_gui).await;
            // We don't care about "official" artifacts of the old GUI, we just need it built.
            // Actually we are interested only in the linked app bundle that is generated by the
            // `ensogl-pack`.
            let _ = old_gui?;
            // We pass the new GUI artifacts to the IDE build.
            new_gui
        }
        .boxed();

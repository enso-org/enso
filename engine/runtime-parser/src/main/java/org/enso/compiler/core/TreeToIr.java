...
final class TreeToIr {
  ...
  Option<Expression> translateInline(Tree.BodyBlock ast) {
    ...
    for (Line statement : ast.getStatements()) {
      ...
      switch (exprTree) {
        ...
        case Tree.Import x -> {}
        case Tree.Invalid x -> {}
        case Tree.TypeSignature sigDeclaration -> {
          Expression sigIr;
          try {
  ...
  private List<IR> translateTypeBodyExpressionImpl(Tree exp, List<IR> appendTo)
    ...
    return switch (inputAst) {
      ...
      case Tree.TypeSignature sig -> join(translateTypeSignature(sig.getSignature()), appendTo);
  ...
  private Definition translateTypeSignature(FunctionTypeSignature sig) {
  ...
  private Definition translateMethodTypeSignature(FunctionTypeSignature sig) throws SyntaxException {
  ...
  private Expression translateTypeSignatureToOprApp(FunctionTypeSignature sig) {
    ...
    switch (tree) {
      case Tree.Assignment assign -> {
        ...
        try {
          ...
          ir = new Expression.Binding(name, expr, getIdentifiedLocation(tree), meta());
        } catch (SyntaxException ex) {
          ir = ex.toError();
        }
        appendTo.add(ir);
      ...
      case Tree.Documented doc -> {
        Expression ir;
        try {
          ir = translateComment(doc.getExpression(), doc.getDocumentation());
        } catch (SyntaxException ex) {
          ir = ex.toError();
        }
        appendTo.add(ir);
  ...
  private IdentifiedLocation getIdentifiedLocation(FunctionTypeSignature sig) {

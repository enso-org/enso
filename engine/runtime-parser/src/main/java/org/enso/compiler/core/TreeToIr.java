...
final class TreeToIr {
  ...
  private Definition translateAnnotation(FunctionAnnotation anno) {
    if (anno.getArgument() == null) {
      return translateSyntaxError(
          getIdentifiedLocation(anno), Syntax.UnexpectedExpression$.MODULE$);
    } else {
  ...
  private Expression translateCall(Tree ast, boolean isMethod) throws SyntaxException {
    ...
    for (; ; ) {
      switch (tree) {
        case Tree.Eval eval -> tree = eval.getValue();
  ...
  private Expression translateExpressionImpl(Tree tree, boolean isMethod) throws SyntaxException {
    ...
    return switch (tree) {
        ...
        for (var l : app.getExpressions()) {
          ...
          if (op == null || isDotOperator(op)) {
            yield translateSyntaxError(
                l.getExpression().getExpression(), Syntax.UnexpectedExpression$.MODULE$);
          }
      ...
      case Tree.Eval eval -> translateExpressionImpl(eval.getValue(), isMethod);
      ...
      case Tree.ArgumentBlockApplication body -> {
        ...
        if (body.getLhs() != null) {
          ...
          for (var line : body.getArguments()) {
            var expr = line.getExpression();
            if (expr instanceof Tree.Eval eval) {
              var value = eval.getValue();
    ...
    while (!done && tree != null) {
      tree =
          switch (tree) {
            ...
            case Tree.Eval eval -> eval.getValue();
  ...
  Expression translateType(Tree tree) {
    return switch (tree) {
      ...
      case Tree.OprApp app -> {
        ...
        yield switch (op.codeRepr()) {
          case "->" -> {
            ...
            if (body == null) {
              yield new Syntax(
                  getIdentifiedLocation(app), Syntax.UnexpectedExpression$.MODULE$, meta());
            }
            var args =
                switch (body) {
      ...
      case Tree.Eval eval -> translateType(eval.getValue());
  ...
  private List<Name> qualifiedNameSegments(Tree t, boolean generateId) throws SyntaxException {
    ...
    if (t instanceof Tree.Eval eval) {
      t = eval.getValue();
    }
    List<Name> names = nil();

 ...
 */
public interface IR {
  /** Compares IR structure, but not metadata neither diagnostics.
   * A special comparator used by <em>Persistance API</em> to perform some rare consistency checks.
   */
  public static final Comparator<IR> STRUCTURE_COMPARATOR = (aIr, bIr) -> {
    if (aIr == bIr) {
      return 0;
    }

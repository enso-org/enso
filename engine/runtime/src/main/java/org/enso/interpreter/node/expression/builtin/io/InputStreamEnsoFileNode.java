...
@BuiltinMethod(type = "File", name = "input_stream", description = "")
public abstract class InputStreamEnsoFileNode extends Node {

  abstract Object execute(EnsoFile _this, Object opts);

  static InputStreamEnsoFileNode build() {
    return InputStreamEnsoFileNodeGen.create();
  }
  ...
  Object execute(EnsoFile _this, Array opts) {
    try {
      ...
      return context
          .getEnvironment()
          .asGuestValue(_this.withTruffleFile(f -> f.newInputStream(hostOpts)));
    } catch (IOException e) {
      Context context = Context.get(this);

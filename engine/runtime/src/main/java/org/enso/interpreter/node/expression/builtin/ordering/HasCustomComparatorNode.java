...
/**
 * Helper node for invocation of {@code Comparable.has_custom_comparator atom}.
...
public abstract class HasCustomComparatorNode extends Node {
   ...
   * and {@code Default_Ordered_Comparator}.
   *
  ...
  boolean hasCustomComparatorCached(
      Atom atom,
      @Cached(value = "getHasCustomComparatorFunction()", allowUncached = true) Function hasCustomComparatorFunc,
    ...
    try {
      ...
      throw new IllegalStateException("Return type from Comparable.has_custom_comparator should be Boolean", e);
    }
  }
  ...
  Function getHasCustomComparatorFunction() {
    ...
    Function hasCustomComparatorFunc =
        comparableType.getDefinitionScope().getMethods().get(comparableType).get("has_custom_comparator");

...
object NoSelfInStatic extends IRPass {
  ...
  override def runModule(
    ...
    ir.copy(
      bindings = ir.bindings.map {
        case method: IR.Module.Scope.Definition.Method.Explicit if isStaticMethod(method) =>
          method.copy(
            body = method.body.transformExpressions(transformSelfToError)
          )
        // TODO: Is this case reachable here?
        case method: IR.Module.Scope.Definition.Method.Binding if isStaticMethod(method) =>
          method.copy(
            body = method.body.transformExpressions(transformSelfToError)
          )
  ...
  private def transformSelfToError: PartialFunction[IR.Expression, IR.Expression] = {
  ...
  private def isStaticMethod(
    ...
    def findSelfArgument(arguments: List[IR.DefinitionArgument]): Option[IR.DefinitionArgument] = {
      arguments.collectFirst {
        case arg @ IR.DefinitionArgument.Specified(IR.Name.Self(_, false, _, _), _, _, _, _, _, _) => arg
      }
    }
    ...
    method.typeName match {
      case Some(_) =>
        method.body match {
          ...
          case body => throw new CompilerError(
   ...
   * @inheritdoc
   */
  override def updateMetadataInDuplicate[T <: IR](sourceIr: T, copyOfIr: T): T = sourceIr
}

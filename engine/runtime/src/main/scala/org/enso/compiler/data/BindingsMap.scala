...
case class BindingsMap(
  ...
  private def findQualifiedImportCandidates(
    ...
    resolvedImports
      .filter(i => importMatchesName(i, name) && !i.isVirtual())
  ...
  private def findExportedCandidatesInImports(
          ...
          imp.module match {
            case ModuleReference.Concrete(module) =>
              module.getIr
                ...
                .map((_, imp.isVirtual(), imp.module))
    ...
    // virtual imports should not be reported in the ambiguity reports
    (resolvedNames match {
      case _ :: _ :: _ =>
...
object BindingsMap {
  ...
  case class ResolvedImport(
    ...
    def isVirtual(): Boolean = {
      importDef.isVirtual && exports.map(_.isVirtual).getOrElse(false)
    }
  }

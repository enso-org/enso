...
class GatherDiagnosticsTest extends CompilerTest {
  ...
  "Error Gathering" should {
    ...
    val plusOp = IR.Name.Literal("+", isReferant = false, None)
    ...
    val lam = IR.Function.Lambda(
      List(
        IR.DefinitionArgument
          .Specified(
            IR.Name.Literal("bar", isReferant = false, None),
            None,
            suspended = false,
            None
          )
    ...
    "work with module flow" in {
      ...
      val typeName    = IR.Name.Literal("Foo", isReferant = false, None)
      val method1Name = IR.Name.Literal("bar", isReferant = false, None)
      val method2Name = IR.Name.Literal("baz", isReferant = false, None)
      val fooName     = IR.Name.Literal("foo", isReferant = false, None)

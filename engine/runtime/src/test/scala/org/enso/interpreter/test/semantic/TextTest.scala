    ...
    "support converting values to display texts" in {
      val code =
        """
          ...
          |    IO.println ((Panic.catch_any (1 + "foo") .convert_to_dataflow_error).catch_primitive .to_display_text)
          |    IO.println ((Panic.catch_any (7 1) .convert_to_dataflow_error).catch_primitive .to_display_text)

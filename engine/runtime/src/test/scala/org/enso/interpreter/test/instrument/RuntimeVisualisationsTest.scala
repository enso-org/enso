...
class RuntimeVisualisationsTest
  ...
  it should "compile and run the visualisation processor used internally by IDE" in {
    val visualisationCode =
    ...
    // TODO: {"jsonrpc":"2.0","id":11,"method":"executionContext/attachVisualisation","params":{"expressionId":"61ca95fc-0a97-4d2c-ab2d-1b632baf1fe7","visualisationConfig":{"executionContextId":"a0f5fb25-f55c-4762-8bfa-ab34306d8038","expression":"\nx ->\n    result = Builtins.Ref.new '{ message: \"\"}'\n    # If x is a PanicSentinel, rethrow it and convert to Error. If x is Error, this keeps it as such.\n    recovered = Builtins.Panic.recover (Builtins.Panic.throw x)\n    recovered.catch err->\n        message = err.to_display_text\n        Builtins.Ref.put result ('{ \"kind\": \"Dataflow\", \"message\": ' + message.to_json.to_text + '}')\n    Builtins.Ref.get result\n","visualisationModule":"Standard.Base.Main"},"visualisationId":"de59a882-d9ca-4ef8-a683-8aad7fa503cd"}}

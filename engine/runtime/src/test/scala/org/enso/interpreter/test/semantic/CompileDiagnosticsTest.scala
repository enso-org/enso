...
class CompileDiagnosticsTest extends InterpreterTest {
  ...
  override def specify(implicit
    ...
    "surface ast-processing errors in the language" in {
      val code =
        """from Standard.Base.Error.Common import all
          ...
          |        case err of
          |            Mk_Syntax_Error msg -> "Oopsie, it's a syntax error: " + msg
          |""".stripMargin
      eval(
        code
    ...
    "surface parsing errors in the language" in {
      val code =
        """from Standard.Base.Error.Common import all
          ...
          |    x.to_text
          |""".stripMargin
      eval(code) shouldEqual "(Make_Syntax_Error 'Unrecognized token.')"
    }
    ...
    "surface redefinition errors in the language" in {
      val code =
        """from Standard.Base.Error.Common import all
          ...
          |""".stripMargin
      eval(code) shouldEqual "(Make_Compile_Error 'Variable x is being redefined.')"
    }
    ...
    "surface non-existent variable errors in the language" in {
      ...
      eval(
        code
      ) shouldEqual "(Make_Compile_Error 'The name `my_vra` could not be found.')"
    }
  }
}

...
/** A request handler for `text/applyExpressionValue` commands.
  *
  ...
  */
class ApplyExpressionValueHandler(
  ...
  private def requestStage: Receive = {
    case Request(
          ApplyExpressionValue,
          id,
          params: ApplyExpressionValue.Params
        ) =>
  ...
  private def responseStage(
    ...
    case RequestTimeout =>
      logger.error(
        "Applying expression value request [{}] for [{}] timed out.",
        id,
        rpcSession.clientId
      )
    ...
    case ApplyEditSuccess =>
      replyTo ! ResponseResult(ApplyExpressionValue, id, Unused)
      cancellable.cancel()
      context.stop(self)
...
object ApplyExpressionValueHandler {

  /** Creates a configuration object used to create a [[ApplyExpressionValueHandler]]
    *
    * @param bufferRegistry a router that dispatches text editing requests
  ...
  def props(
    ...
    Props(
      new ApplyExpressionValueHandler(
        bufferRegistry,
        requestTimeout,
        rpcSession
      )

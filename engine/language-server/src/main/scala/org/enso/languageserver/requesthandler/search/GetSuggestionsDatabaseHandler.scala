...
/**
  ...
  * @param searchHandler a reference to the search handler
  */
class GetSuggestionsDatabaseHandler(
  timeout: FiniteDuration,
  searchHandler: ActorRef
) extends Actor
    with ActorLogging
  ...
  private def requestStage: Receive = {
    case Request(GetSuggestionsDatabase, id, _) =>
      searchHandler ! SearchProtocol.GetSuggestionsDatabase
      val cancellable =
  ...
  private def responseStage(
    ...
    case SearchProtocol.SuggestionsDatabaseFailure() =>
...
object GetSuggestionsDatabaseHandler {
  ...
  def props(
    ...
    Props(new GetSuggestionsDatabaseHandler(timeout, searchHandler))

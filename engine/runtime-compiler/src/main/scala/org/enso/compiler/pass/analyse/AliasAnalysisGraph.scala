...
sealed class AliasAnalysisGraph extends Serializable {
  ...
  private var globalSymbols: Map[AliasAnalysisGraph.Symbol, Occurrence.Global] = Map()
    ...
    scopeFor(occurrence.id) match {
      case Some(scope) =>
        globalSymbols
          ...
          .map(g => AliasAnalysisGraph.Link(occurrence.id, scope.scopesToRoot + 1, g.id))
      case None => None
    }
  }
  ...
  def knownShadowedDefinitions(
    ...
    def getShadowedIds(scope: AliasAnalysisGraph.Scope): Set[AliasAnalysisGraph.Occurrence] = {
   ...
   */
  def idToSymbol(id: AliasAnalysisGraph.Id): Option[AliasAnalysisGraph.Symbol] = {

...
abstract class ImportResolverForIR
  ...
  protected final java.util.List<Export.Module> exportsFor(Module module, String impName) {
    java.util.List<Export.Module> exp =
        CollectionConverters.SeqHasAsJava(module.exports()).asJava().stream()
            .map(
                e ->
                    switch (e) {
                ...
                e ->
                    switch (e) {
                      case BindingsMap.Type t -> {
                        ...
                        var res =
                            new ResolvedType(new BindingsMap$ModuleReference$Concrete(mod), t);
                        assert e.name().equals(res.tp().name())
                            : e.name() + " != " + res.tp().name();
                        yield res;
                      }
                      case null, default -> null;
   ...
   *
   * @param imp The import is treated as an import of a constructor from a type. The last part is
   *     constructor, the second to last is type, the third to last is module.
  ...
  protected java.util.List<ResolvedConstructor> definedConstructors(Import.Module imp) {
    ...
    var modFullName = parts.stream().limit(parts.size() - 2).collect(Collectors.joining("."));
    ...
    var bindingsMap = loadBindingsMap(mod);
    var foundType =
        scala.jdk.javaapi.CollectionConverters.asJava(bindingsMap.definedEntities()).stream()
            .map(
                e -> {
    ...
    var resolvedType =
        new BindingsMap.ResolvedType(new BindingsMap$ModuleReference$Concrete(mod), tp);
    return scala.jdk.javaapi.CollectionConverters.asJava(tp.members()).stream()
  ...
  protected java.util.List<ResolvedModuleMethod> definedModuleMethods(Import.Module imp) {
    ...
    var modFullName = parts.stream().limit(modMethodNameIdx).collect(Collectors.joining("."));
    var compiler = getCompiler();
    var optionMod = compiler.getModule(modFullName);
    if (optionMod.isEmpty()) {
      return null;
    ...
    var modMethods =
        scala.jdk.javaapi.CollectionConverters.asJava(bindingsMap.definedEntities()).stream()
            ...
            .map(
                entity ->
                    new ResolvedModuleMethod(
                        new BindingsMap$ModuleReference$Concrete(mod),
                        (BindingsMap.ModuleMethod) entity))
            .collect(Collectors.toUnmodifiableList());
    return modMethods;
  }
  ...
  protected java.util.List<ResolvedExtensionMethod> definedExtensionMethods(Import.Module imp) {
    ...
    var extensionMethods =
        scala.jdk.javaapi.CollectionConverters.asJava(bindingsMap.definedEntities()).stream()
            ...
            .map(
                entity ->
                    new ResolvedExtensionMethod(
                        new BindingsMap$ModuleReference$Concrete(mod),
                        (BindingsMap.ExtensionMethod) entity))
            .collect(Collectors.toUnmodifiableList());
  ...
  protected java.util.List<ResolvedConversionMethod> definedConversionMethods(Import.Module imp) {
    ...
    var conversionMethods =
        scala.jdk.javaapi.CollectionConverters.asJava(bindingsMap.definedEntities()).stream()
            ...
            .map(
                entity ->
                    new ResolvedConversionMethod(
                        new BindingsMap$ModuleReference$Concrete(mod),
                        (BindingsMap.ConversionMethod) entity))
            .collect(Collectors.toUnmodifiableList());
    return conversionMethods;
  }

  @Override
  protected final CompilerContext.Module loadLibraryModule(
      LibraryName libraryName, String moduleName) throws IOException {
    var compiler = this.getCompiler();
  ...
  @Override
  protected final Tuple2<Import, Option<BindingsMap.ResolvedImport>> createResolvedImport(
    ...
    var resolvedImport =
        new BindingsMap.ResolvedImport(
            imp, toScalaList(exp), toScalaList(java.util.List.of(resolvedModule)));
    return new Tuple2<>(imp, scala.Some.apply(resolvedImport));
  }

  @Override
  protected final Tuple2<Import, Option<BindingsMap.ResolvedImport>> createResolvedType(
      Import.Module imp, java.util.List<Export.Module> exp, BindingsMap.ResolvedType typ) {
    var resolvedImport =
        new BindingsMap.ResolvedImport(imp, toScalaList(exp), toScalaList(java.util.List.of(typ)));
    return new Tuple2<>(imp, scala.Some.apply(resolvedImport));
  }

  @Override
  protected Tuple2<Import, Option<ResolvedImport>> createResolvedConstructor(
      Import.Module imp, java.util.List<Export.Module> exp, ResolvedConstructor cons) {
    var resolvedImport =
        new BindingsMap.ResolvedImport(imp, toScalaList(exp), toScalaList(java.util.List.of(cons)));
    return new Tuple2<>(imp, scala.Some.apply(resolvedImport));
  }

  @Override
  protected Tuple2<Import, Option<ResolvedImport>> createResolvedModuleMethod(
      Import.Module imp,
      java.util.List<Export.Module> exp,
      ResolvedModuleMethod resolvedModuleMethod) {
    var resolvedImport =
        new BindingsMap.ResolvedImport(
            imp, toScalaList(exp), toScalaList(java.util.List.of(resolvedModuleMethod)));
    return new Tuple2<>(imp, scala.Some.apply(resolvedImport));
  }

  @Override
  protected Tuple2<Import, Option<ResolvedImport>> createResolvedExtensionMethods(
      Import.Module imp,
      java.util.List<Export.Module> exp,
      java.util.List<ResolvedExtensionMethod> extensionMethods) {
    java.util.List<ImportTarget> importTargets =
        extensionMethods.stream()
            .map(ImportTarget.class::cast)
            .collect(Collectors.toUnmodifiableList());
    var resolvedImport =
        new BindingsMap.ResolvedImport(imp, toScalaList(exp), toScalaList(importTargets));
    return new Tuple2<>(imp, scala.Some.apply(resolvedImport));
  }
  ...
  protected Tuple2<Import, Option<ResolvedImport>> createResolvedConversionMethods(
    ...
    java.util.List<ImportTarget> importTargets =
        resolvedConversionMethods.stream()
            .map(ImportTarget.class::cast)
            .collect(Collectors.toUnmodifiableList());
    var resolvedImport =
        new BindingsMap.ResolvedImport(imp, toScalaList(exp), toScalaList(importTargets));
  ...
  protected final Tuple2<Import, Option<BindingsMap.ResolvedImport>>
    ...
    org.enso.compiler.core.ir.expression.errors.ImportExport importError =
        new ImportExport(
            imp,
            new ImportExport.PackageCouldNotBeLoaded(impName, loadingError),
            imp.passData(),
            imp.diagnostics());
  ...
  @Override
  protected final Tuple2<Import, Option<BindingsMap.ResolvedImport>> createErrorModuleDoesNotExist(
      Import.Module imp, String impName) {
    return new Tuple2<>(
        new ImportExport(
            imp, new ImportExport.ModuleDoesNotExist(impName), imp.passData(), imp.diagnostics()),
        Option.empty());
  }

...
final class JobExecutionEngine(
  ...
  private val pendingCancellationsExecutor =
    context.newFixedThreadPool(1, "pending-cancellations")
  ...
  val jobExecutor: ExecutorService =
    context.newFixedThreadPool(jobParallelism, "job-pool")
  ...
  private val backgroundJobExecutor: ExecutorService =
    context.newCachedThreadPool("background-job-pool", 1, 4, MaxJobLimit)

 ...
 * <p>
 * This parser will attempt to work out what the decimal point and thousand separators used in the input. It will try
 * various ways of formatting a number and can be set to allow for scientific notation, currency symbols.
 * <p>
 * If parsing a column it will select the format that parses the longest set without an issue from the top and then
 * apply this format to all the rows.
 * <p>
 * The separators will be tried in British, German, French and Swiss order. - Thousand separator must be followed by
 * groups of 3 numbers. - Scientific notation is only allowed on decimals and must be on a value between -10 and 10. The
 * notation is an `E` followed by an integer.
 * <p>
 * The following formats are supported: - Sign (+/-) followed by Number (e.g. +1,234.56) - Using brackets to indicate a
 * negative number (e.g. (1,234.56)) - Currency symbols (if enabled) can be placed before or after the sign and number.
...
public class NumberParser extends IncrementalDatatypeParser {
  ...
  private static void validateSeparator(String name, String value) {
    ...
    // If we allowed separators to be a digit, super crazy stuff could happen - e.g. technically 10000 could be
      // interpreted as 1000 by interpreting the first 0 as a thousand separator. Let's not do that.
   ...
   * <p>
   * If one of the parameters is null, it is meant to be inferred (multiple separator pairs will be provided for it), if
  ...
  private static Separators[] buildSeparators(boolean allowDecimal, String decimalPoint, String thousandSeparator) {
    ...
    if (decimalPoint != null && decimalPoint.equals(thousandSeparator)) {
      throw new IllegalArgumentException("Decimal point and thousand separator cannot be the same, but they were both" +
          " '" + decimalPoint + "'.");
    }
  ...
  private static Pattern buildPattern(boolean allowDecimal, boolean allowCurrency, boolean allowScientific,
                                      boolean trimValues, int patternIndex, Separators separators) {
    ...
    String INTEGER = "(?<integer>(\\d*)" + (separators.thousand == null ? "" :
  ...
  public static NumberParser createIntegerParser(IntegerType integerTargetType, boolean allowCurrency,
    ...
    return new NumberParser(false, integerTargetType, allowCurrency, allowLeadingZeros, trimValues, false,
   ...
   * @param thousandSeparator the thousand separator to use (if null, then will be inferred)
   */
  public static NumberParser createDecimalParser(boolean allowCurrency, boolean allowLeadingZeros, boolean trimValues
      , boolean allowScientific, String decimalPoint, String thousandSeparator) {
    return new NumberParser(true, null, allowCurrency, allowLeadingZeros, trimValues, allowScientific, decimalPoint,
        thousandSeparator);
  }

  private NumberParser(boolean allowDecimal, IntegerType integerTargetType, boolean allowCurrency,
                       boolean allowLeadingZeros, boolean trimValues, boolean allowScientific, String decimalPoint,
  ...
  /**
   * Creates a Pattern for the given index. The index will be decoded into a specific set of separators (unless fixed
   * separators are used) and then paired with one of the valid patterns for the given parser.
   */
  private Pattern patternForIndex(int index) {
    ...
    return buildPattern(allowDecimal, allowCurrency, allowScientific, trimValues, patternIndex,
  ...
  private WithAggregatedProblems<Storage<?>> sealBuilderAndMergeProblems(Builder builder,
                                                                         ProblemAggregator aggregator) {
  ...
  private int parseColumnWithPattern(Pattern pattern, Storage<String> sourceStorage, Builder builder,
                                     ProblemAggregator aggregator) {

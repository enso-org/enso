...
public class Last extends Aggregator {
  ...
  public Last(String name, Column column, Column[] ordering, boolean ignoreNothing) {
    ...
    if (ordering.length == 0) {
      ...
      for (int row : indexes) {
        ...
        MultiValueKey newKey =
            new MultiValueKey(Arrays.stream(this.ordering).map(o -> o.getItemBoxed(row)).toArray());

...
public class ExcelWriter {
    ...
    private static void appendRangeWithTable(
        ...
        Table mappedTable
            ...
            case APPEND_BY_INDEX ->
                ColumnMapper.mapColumnsByPosition(
     ...
     * @return True if range is empty and clear is False, otherwise returns
     * False.
     */
    ...
    private static void writeValueToCell(Cell cell, int j, Storage<?> storage, Workbook workbook)
        ...
        if (storage.isNothing(j)) {
            ...
            var dateTime = dateTimeStorage.getItem(j).toLocalDate();
            if (dateTime.isBefore(LocalDate.of(1900, 1, 1))) {
    ...
    /**
     * Determines if headers should be written for the given range in
     * {@code INFER} mode.
     *
     * <p>
     * Unlike in the {@code ExcelReader}, if empty this will default to having
     * headers.
     *
     ...
     * @param endCol end column index (1-based) of the range to check. If -1
     * will continue until end of row.
     * @return EXCEL_COLUMN_NAMES if the range has headers, otherwise

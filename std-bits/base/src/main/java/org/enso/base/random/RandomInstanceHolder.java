...
/**
 * Container for a `Random` object. Setting the seed requires recreating the
 * `Random` object, so the `Random` should always be obtained through
 * `getCurrentRandom`.
 */
public class RandomInstanceHolder {
    ...
    //public RandomInstanceHolder getSingleton() { return SINGLETON; }
}
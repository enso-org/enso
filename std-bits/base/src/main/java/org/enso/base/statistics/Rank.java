...
public class Rank {
  ...
  private static double[] computeRankFromTuples(
      List<ValueWithIndex> tuples, Comparator<Object> comparator, Method method)
      throws NullPointerException, ClassCastException {
    Comparator<ValueWithIndex> tupleComparator =
        (a, b) -> {
    ...
    while (index < tuples.size()) {
      ...
      while (index < tuples.size()
          && comparator.compare(tuples.get(start).value, tuples.get(index).value) == 0) {
        index++;
      }

...
public class Regex_Utils {
   ...
   *
   * Special regex characters present in the input pattern are quoted to match them literally according to the SQL-like format.
   */
  public static String sql_like_pattern_to_regex(String sql_pattern) {
    ...
    for (int i = 0; i < sql_pattern.length(); ++i) {
      ...
      if (c == '%' || c == '_') {
        // Before inserting the converted wildcard, we append the accumulated characters, quoting them first.
        if (acc.length() > 0) {

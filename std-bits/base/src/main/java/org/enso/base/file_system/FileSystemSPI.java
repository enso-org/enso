...
public abstract class FileSystemSPI {
  private static final ServiceLoader<org.enso.base.file_system.FileSystemSPI> loader =
      ServiceLoader.load(org.enso.base.file_system.FileSystemSPI.class, org.enso.base.file_format.FileFormatSPI.class.getClassLoader());
  ...
  public static Value get_type(String protocol, boolean refresh) {
    ...
    var first = loader.stream().filter(provider -> provider.get().getProtocol().equals(protocol)).findFirst();
    return first.map(fileSystemSPIProvider -> fileSystemSPIProvider.get().getTypeObject()).orElse(null);
  }

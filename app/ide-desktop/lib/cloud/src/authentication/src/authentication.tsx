...
export const AuthProvider = (props: AuthProviderProps): JSX.Element => {
    ...
    const [authSession] = useAsyncEffect(
        null,
        logger,
        async () => {
    ...
    useEffect(() => {
        const fetchSession = async () => {
            // FIXME [NP]: remove this log
            console.log(
                `AuthProvider::useEffect::fetchSession::${JSON.stringify(authSession)}::${refresh}`
            )
        ...
        try {
            ...
            if (isAmplifyError(error)) {
                if (error.code === 'UserNotFoundException') {
                    ...
                    toast.error(
                        'User not confirmed. Please check your email for a confirmation link.'
                    )
                    return
    ...
    logger.log(
        `authProvider::render::initialized ${initialized}::refresh ${refresh}::session ${JSON.stringify(
            session
        )}`
    )
    return (
...
//export const ProtectedRoute: FC<ProtectedRouteProps> = ({ isAllowed, redirectPath = LOGIN_PATH, children }): ReactNode => {
// eslint-disable-next-line @typescript-eslint/naming-convention
export const ProtectedRoute2: FC<ProtectedRouteProps> = ({
    isAllowed,
    redirectPath = LOGIN_PATH,
    children,
}) => {
...
// eslint-disable-next-line @typescript-eslint/naming-convention
export const withRedirectIfUnauthorized =
    (config: Config) =>

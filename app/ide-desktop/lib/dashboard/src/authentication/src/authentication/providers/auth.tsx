...
interface AuthContextType {
    ...
    forgotPassword: (email: string, needJump?: boolean) => Promise<void>
...
export const AuthProvider = (props: AuthProviderProps) => {
    ...
    const forgotPassword = async (email: string, needJump = false) =>
        cognito.forgotPassword(email).then(result => {
            if (result.ok) {
                toast.success(MESSAGES.forgotPasswordSuccess)
                if (needJump) navigate(app.RESET_PASSWORD_PATH)
            } else {
                toast.error(result.val.message)
            }
        })

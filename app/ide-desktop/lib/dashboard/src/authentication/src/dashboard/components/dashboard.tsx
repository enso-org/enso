...
import { useProjectManageAdapter } from '../useProjectManagerAdapter'
...
const dashboardContainer = (props: DashboardProps) => {
    ...
    const handleCreateProject = async (templateName: string | undefined) => {
        ...
        const newProject = await projectManagerAdapter.createProject(newProjectName, templateName)
    ...
    if (projectsList.length > 0) {
        const setProjectOpening = (projectItemIndex: number): void => {
            setProjectsList(currProjectList => {
                ...
                newProjectList[projectItemIndex]!.state.type = backend.ProjectState.openInProgress

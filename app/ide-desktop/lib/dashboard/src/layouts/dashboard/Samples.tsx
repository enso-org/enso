    ...
    <div className="flex flex-col gap-1.5 h-51">
      ...
      >
        <div
          ...
          className={`rounded-t-2xl w-full h-25 ${template.background != null ? '' : 'bg-frame'}`}
        />
        <div className="grow bg-frame backdrop-blur rounded-b-2xl w-full px-4 pt-1.75 pb-3.5">
          ...
          <div className="text-xs text-ellipsis leading-144.5 pb-px">{template.description}</div>
        </div>
        {spinnerState != null && (
    ...
    <div className="flex flex-col gap-4 px-4.75">
      ...
      <div className="grid gap-2 grid-cols-fill-60">
        ...
        {SAMPLES.map(template => (
          <ProjectTile key={template.id} template={template} onTemplateClick={onTemplateClick} />
        ))}
      </div>
    </div>
  )

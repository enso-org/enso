...
export default function AccountSettingsTab() {
  ...
  const doUploadUserPicture = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (image == null) {
      ...
      // Ignore. If `user` is `null`, all other endpoints will not work either.
    } else {
      try {
    ...
    <div className="flex gap-8">
                  ...
                  onKeyDown={event => {
                    switch (event.key) {
                      case 'Escape': {
                        ...
                        event.currentTarget.value = user?.value.name ?? ''
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <label className="flex items-center cursor-pointer rounded-full overflow-clip h-32 w-32 hover:bg-frame transition-colors">
          ...
          <img src={user?.value.profilePicture ?? DefaultUserIcon} width={128} height={128} />
        </label>

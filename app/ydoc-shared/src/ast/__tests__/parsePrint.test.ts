...
import { spanMapToIdMap } from '../idMap'
...
import { BodyBlock } from '../tree'

test('Raw block abstracts to BodyBlock', () => {
  const code = 'value = 2 + 2'
  const rawBlock = rawParseModule(code)
  const edit = MutableModule.Transient()
  const abstracted = abstract(edit, rawBlock, code)
  expect(abstracted.root).toBeInstanceOf(BodyBlock)
})
...
const normalizingCases = [
  // Leading space before first line
  { input: ' a', normalized: '    a' },
  // Trailing space after last line
  { input: 'a ', normalized: 'a \n' },
]
test.each([
  'Console.',
  ...
  'a\n## | A   | B   |\n   \n   | 5   | 6   |\nmain = 23\n',
  ...normalizingCases,
])('parse/print round-trip: %s', (testCase) => {
  ...
  expect(repair(root).fixes).toBe(undefined)

  // Re-parse.
  const { root: root1, spans: spans1 } = parseModuleWithSpans(printed.code)
  setExternalIds(root1.module, spans1, idMap)
  // Check that Identities match original AST.
  const printed1 = printWithSpans(root1)
  printed1.info.tokens.clear()
  const idMap1 = spanMapToIdMap(printed1.info)

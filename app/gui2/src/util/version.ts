...
export async function checkMinimumSupportedVersion(config: ApplicationConfigValue) {
  let supported = false
  if (config.engine.skipMinVersionCheck) {
    supported = true
  } else {
    try {
      const appConfig = await fetchTimeout(config.engine.configUrl, FETCH_TIMEOUT)
      if (

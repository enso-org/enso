...
const xLabelLeft = computed(() => boxWidth.value / 2 + getTextWidthBySizeAndFamily(axis.value.x?.label) / 2)
const yLabelTop = computed(() => -margin.value.left + AXIS_LABEL_HEIGHT)
const yLabelLeft = computed(() => -boxHeight.value / 2 + getTextWidthBySizeAndFamily(axis.value.y?.label) / 2)

...
const placementEnvironment = computed<Environment>(() => {
  ...
  return { mousePosition, nodeRects, selectedNodeRects, screenBounds }
})
...
function getNodeContent(id: ExprId): string {
  return graphStore.db.nodes.get(id)?.rootSpan.repr() ?? ''
}

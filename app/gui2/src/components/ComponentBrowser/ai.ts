...
export function useAI() {
  ...
  async function query(query: string, sourceNode: Node): Promise<Result<string>> {
    ...
    prompt.value.replace(AI_GOAL_PLACEHOLDER, query)
    console.log('Prompt for the AI: ', prompt.value)
    const { code } = await lsRpc.aiCompletion(prompt.value, AI_STOP_SEQUENCE)

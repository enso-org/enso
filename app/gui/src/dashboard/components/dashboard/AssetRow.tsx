...
export const AssetRow = React.memo(
  function AssetRow(props: AssetRowProps) {
    ...
    const { data: projectState } = useQuery({
      ...createGetProjectDetailsQuery.createPassiveListener(
        ...
        item.item.id as backendModule.ProjectId,
      ),
  ...
  (oldProps, newProps) => {
    for (const k of object.unsafeKeys<AssetRowProps>(oldProps)) {
      switch (k) {
        case 'item': {
          return object.deepEqual(oldProps.item, newProps.item)
        }
        default: {
          if (!Object.is(oldProps[k], newProps[k])) {
            return false
          }
        }
      }

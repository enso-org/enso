 ...
 */
export function resolveDocImageResource(modulePathSegments: string[], imageUrl: string) {
  const appliedUrl = URL.parse(imageUrl)
  switch (appliedUrl?.protocol) {
    ...
    case 'enso:':
      return Ok({ type: 'ensoPath' as const, ensoPath: EnsoPath(decodeURI(appliedUrl.href)) })
    default:

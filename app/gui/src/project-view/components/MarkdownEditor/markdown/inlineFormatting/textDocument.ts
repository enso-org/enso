...
export class TextDocument {
  ...
  trimRangeSpaces(range: Readonly<DelimitersTrimmedRange>): TrimmedRange {
    return {
      from: range.from + spacesAfter(this.lineContentAfter(range.from)),
      to: range.to - spacesBefore(this.lineContentBefore(range.to)),
    } as TrimmedRange
  }
  ...
  expandRangeSpaces(range: Readonly<Range>): Range {
    return {
      from: range.from - spacesBefore(this.lineContentBefore(range.from)),
      to: range.to + spacesAfter(this.lineContentAfter(range.to)),
    } as TrimmedRange
  }
  ...
  wordAt(pos: number): TrimmedRange {
    ...
    return {
      from: pos - nonSpacesBefore(textBefore),
      to: pos + nonSpacesAfter(textAfter),
    } as TrimmedRange
  }

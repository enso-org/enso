...
impl Model {
    ...
    fn setup_searcher_presenter(&self, node_id: NodeId, source_node: Option<NodeId>) {
...
impl Project {
    ...
    fn init(self) -> Self {
        ...
        frp::extend! { network
            eval view.searcher_opened (((node, source_node)) {
                model.setup_searcher_presenter(*node, *source_node)
            });

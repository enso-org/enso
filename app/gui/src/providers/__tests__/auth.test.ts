...
import { createAuthStore, makeSyntheticUser } from '../auth'
...
async function setupAuthStore({
  ...
  return { store: store!, sessionData }
}
...
describe('auth store while the Cloud is unreachable', () => {
  ...
  it('does not degrade while signing out', async () => {
    const { store, sessionData } = await setupAuthStore({ isCognitoUnreachable: true })
  ...
  it('recovers once the Cloud answers again', async () => {
    ...
    sessionData.isCloudUnreachable = false

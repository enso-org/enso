...
function createAuthStore(
   ...
   * with a non-auth error. Auth (401/403) failures keep the existing sign-out and
   * redirect handling and are excluded here. Requires a local backend so the synthesised
   * session has somewhere to land — on cloud-only deployments without `localBackend`,
   * this stays `false` and the user falls through to the existing redirect-to-login path.
   */

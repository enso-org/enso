...
//                 );
//                 is_scrolled <- scroll_area.scroll_position_y.map(f!([this] (s) *s >
// this.heights_sum(i)));                 change_hdr_pos <-
// scroll_area.scroll_position_y.gate(&is_scrolled);                 reset_hdr_pos <-
// is_scrolled.on_false();                 eval change_hdr_pos([group, this](y)
// group.set_header_pos(*y - this.heights_sum(i)));                 eval_
// reset_hdr_pos(group.set_header_pos(0.0));             }
//         }
//     }
//
...
//         eval selected_entry([](e) if let Some(e) = e {  DEBUG!("Entry {e.1} from group {e.0}
// selected") });         eval multiview.suggestion_accepted([]((g, s)) DEBUG!("Suggestion {s}
// accepted in group {g}"));         eval multiview.expression_accepted([]((g, s))
// DEBUG!("Expression {s} accepted in group {g}"));         header_selected <-
// multiview.is_header_selected.filter_map(|(g, h)| if *h { Some(*g) } else { None });         eval
// header_selected([](g) DEBUG!("Header selected in group {g}"));         eval
// multiview.header_accepted([](g) DEBUG!("Header accepted in group {g}"));
//

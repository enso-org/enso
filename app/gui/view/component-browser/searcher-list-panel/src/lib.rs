...
use ensogl_hardcoded_theme::application::searcher::icons::favorites;
...
impl Model {
    ...
    fn update_layers(&self) {
        self.favourites_section.set_layers(&self.layers.groups, self.scroll_area.content_layer());
        self.local_scope_section.set_layers(&self.layers.groups, self.scroll_area.content_layer());
        self.sub_modules_section.set_layers(&self.layers.groups, self.scroll_area.content_layer());
    }
...
trait SectionContent: display::Object {
    fn set_layers(&self, layers: &GroupLayers, scroll_layer: &Layer);
...
impl SectionContent for component_group::wide::View {
    fn set_layers(&self, layers: &GroupLayers, _scroll_layer: &Layer) {

...
impl Model {
    ...
    #[profile(Debug)]
    fn build_port_shapes_on_new_expression(&self, expression: &mut Expression, frp: &FrpEndpoints) {
        ...
        expression.span_tree.root_ref_mut().dfs_with_layer_data(builder, |mut node, builder| {
            ...
            let new_parent = if not_a_port {
                ...
                frp::extend! { port_network
                    ...
                    frp.source.body_hover <+ bool(&mouse_out,&mouse_over_raw);
                    ...
                    frp.source.on_background_press <+ bg_down;


                    // === Press ===

                    frp.source.on_port_press <+ mouse_down.map(f_!([crumbs] crumbs.clone_ref()));
                    ...
                    frp.source.pointer_style <+ pointer_style;
                }
                init_color.emit(());
                frp.set_view_mode.emit(frp.view_mode.value());
    ...
    fn init_port_frp_on_new_expression(
        ...
        expression.root_ref_mut().dfs_with_layer_data(parent_tp, |node, parent_tp| {
            ...
            let styles = self.styles.clone_ref();
            let styles_frp = self.styles_frp.clone_ref();
            ...
            if node.children.is_empty() {
                ...
                let label = self.label.clone_ref();
                frp::extend! { port_network
    ...
    fn set_expression(
        &self,
        new_expression: &node::Expression,
        is_editing: bool,
        frp: &FrpEndpoints,
    ) -> Expression {
        let mut new_expression = Expression::from(new_expression.clone());
        if DEBUG {
            DEBUG!("\n\n=====================\nSET EXPR: " new_expression.code)
        }
        self.label.set_content(&new_expression.viz_code);
        self.build_port_shapes_on_new_expression(&mut new_expression, frp);
        self.init_port_frp_on_new_expression(&mut new_expression, frp);

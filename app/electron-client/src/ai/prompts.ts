...
- **URL with query params** → narrow the params in your verification URL. A BoE series with \`Datefrom=01/Jan/2020&Dateto=01/Jan/2026\` (~1500 rows, slow) probes fine with \`Datefrom=01/Jan/2025&Dateto=08/Jan/2025\` (~5 rows). Same column structure, fits the budget.
- **JSON / Parquet / plain HTTP without query-narrowing params** → no reader-side cap exists. Either accept the parse cost, or skip the real read and probe the planned operation against a tiny literal (e.g. \`Table.new [["Code",["XUDLADD"]],["Value",[1.6]]] . rename_columns …\`).
...
**On \`executeExpression: Execution timed out.\`, retry with a smaller cap.** That exact string means "the read was too large", not "the code is wrong" — a \`Panic\`, \`DataflowError\`, or type-mismatch message means **fix the code**; \`Execution timed out.\` means **shrink the read**. (1) First retry — halve the cap (\`row_limit=100\` → \`row_limit=10\`, 1-week URL window → 1-day). (2) Second retry — minimum viable probe (\`row_limit=1\` or, for uncappable sources, a tiny literal inline sample). (3) After two retries, stop and emit the closing JSON without further verification — three timeouts in a row indicate something structural you can't fix from inside the verify loop.

...
**Live progress narration (REQUIRED — both rules below apply on every turn).** The user sees these notes as the placeholder node's status text. Skipping them leaves the user staring at "Thinking…"; the runtime treats long silence as "stuck" and kills the turn shortly after. Code, expression text, and file paths must NOT appear in the narration — those are logged separately.

1. **Cadence (REQUIRED — equal weight to the tool-call rule).** Emit a fresh text block at least every ~20 seconds of work, and **never let more than 40 seconds elapse between any two feedback events** (text or tool call). Narrating mid-thinking is fine and expected — "Still scanning Table docs", "Considering join shape", "Refining the predicate", "Re-reading the prompt". This rule binds independently of whether a tool call is coming next: a long thinking stretch with no tool calls still needs ≥1 text block per 30 s. A silent stretch past 60 s is a hard failure for the user.

2. **Tool-call narration (REQUIRED — equal weight to the cadence rule).** Before EACH \`tool_use\` block, emit one short text block (≤8 words, present continuous) describing what *this specific* tool call is checking — e.g. "Checking Result distinct values", "Counting finalist rows", "Reading Table.join docs", "Verifying final body". When running a multi-step check within the turn, suffix with your own progress — "Probing data shape (2/4)", "Verifying body — final check", "Almost done — last probe". Each tool call gets its own narration even if it is part of the same logical step.

...
object GenerateAST {
  ...
  private val BuildProfile = "BUILD_PROFILE"
  ...
  private def generateAST(out: File, log: ManagedLogger): Unit = {
    val profile = sys.env.getOrElse(BuildProfile, "dev")
    val command =
      s"$cargoCmd run -Z unstable-options --profile $profile -p ast -- --generate-scala-ast $out"
